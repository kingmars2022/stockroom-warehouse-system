...
type Audit = { id: string; actor: string; role: Role; action: string; target: string; detail: string; at: string };
...
const seed: Store = {
  ...
  audits: [
    { id: 'a1', actor: 'Alex Chen', role: 'employee', action: 'Issued stock', target: 'USB-C charging cable', detail: '4 pcs issued to Design team', at: '2026-08-25T09:42:00' },
    { id: 'a2', actor: 'Mia Wong', role: 'supervisor', action: 'Received purchase', target: 'Shipping box, medium', detail: '120 pcs from Northstar Supply', at: '2026-08-25T08:15:00' },
    ...
    { id: 'a4', actor: 'Mia Wong', role: 'supervisor', action: 'Approved reimbursement', target: '4 x 6 shipping labels', detail: 'Approved $39.60 submitted by Alex Chen', at: '2026-08-23T09:45:00' },
    { id: 'a5', actor: 'Chen Manager', role: 'admin', action: 'Reviewed price alert', target: 'USB-C charging cable', detail: 'Northstar Supply price increased by 20%', at: '2026-08-25T08:30:00' },
  ],
};
...
export default function Home() {
  ...
  return <I18nContext.Provider value={i18n}><main className="app-shell">
    ...
    {dialog === 'purchase' && <PurchaseDialog items={store.items} suppliers={store.suppliers} recommendations={store.replenishment} close={() => setDialog(null)} submit={addPurchase} />}
...
function PurchaseDialog({ items, suppliers, recommendations, close, submit }: { items: Item[]; suppliers: Supplier[]; recommendations: Replenishment[]; close: () => void; submit: (event: FormEvent<HTMLFormElement>) => void }) {
  const [itemId, setItemId] = useState('');
